...
export default function Environmentals() {
  const classes = useStyles()
  // Get All Domains
  const [domain, setDomain] = useState([])
  const getDomains = async () => {
    try{
      const domains = await axios.get("http://ec2-52-90-134-187.compute-1.amazonaws.com/metadata/getdomainsenvironmental");
      ...
      <Paper elevation={3} style={{height: '100%',  width:"100%"}}>
        <TableContainer component={Paper}>
          <Table className={classes.table}>
            <TableHead>
              <TableRow>
                <StyledTableCell align="center">MTD_ID</StyledTableCell>
            ...
            <TableBody>
              {domain.map((row) => (
                <StyledTableRow key={row.Module}>
                  <StyledTableCell align="center">{row.mtd_id_list.map((item) => <div>{}</div>)}</StyledTableCell>
