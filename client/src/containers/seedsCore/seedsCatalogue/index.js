...
export default function DataCatalogue() {
  ...
  useEffect(() => {
    const fetchData = async() => {
      const res4 = await axios('http://ec2-52-90-134-187.compute-1.amazonaws.com/metadata/',);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      const saveSession = async() => {
        const res = await axios.post("http://ec2-52-90-134-187.compute-1.amazonaws.com/session/edit", sessionData)
        .then(function(res) {
          if ('errors' in res) {
  ...
  const handleClose = (loaded) => {  //para ma-display yung data sa mapa
    ...
    const fetchData = async (i) =>{
      const res = await axios.get(`http://ec2-52-90-134-187.compute-1.amazonaws.com/getdata/`,{params:{id: selected[i]}})
      const res2 = await axios.get(`http://ec2-52-90-134-187.compute-1.amazonaws.com/getdata/sld`,{params:{metadataID: selected[i]}})
  ...
  const handleAddToMap = async () => {
    ...
    var loaded = []
    const res = await axios.get(`http://ec2-52-90-134-187.compute-1.amazonaws.com/metadata/checkData`,{params:{selected: selected}})
  ...
  const DeleteDialog = () => {
    ...
    const handleDeleteData = () => {
      if (textFieldContent === selected[0]) {
        ...
        setOpenBackdrop(true)
        axios.post('http://ec2-52-90-134-187.compute-1.amazonaws.com/getdata/delete', {"id": textFieldContent})
        .then(async (res) => {
          const res4 = await axios('http://ec2-52-90-134-187.compute-1.amazonaws.com/metadata/',); //ito yung gagamitin pag sa web yung server
          setDataCat(res4.data);
          setDataShow(res4.data);
